 ...
 * 
 * SECURITY NOTE: This service performs privileged operations that bypass RLS.
 * It should only be used with a server-side Supabase client (service role key).
 * See: src/lib/supabase/server-client.ts
 */
 ...
 * @param rowNumber - The Google Sheets row number
 */
 ...
 * @param supabase - Server-side Supabase client with service role key
 */
export async function processNewSubmissions(supabase: ServerSupabaseClient) {
